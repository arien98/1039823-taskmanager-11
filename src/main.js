...
renderElement(`.board__tasks`, createCardFormTemplate(tasks[0]), `beforeend`);

let showingTasksCount = SHOWING_TASKS_COUNT_ON_START;
tasks.slice(1, showingTasksCount).forEach((task) => renderElement(`.board__tasks`, createCardTemplate(task), `beforeend`));
...
const clickMoreButtonHandler = () => {
  ...
  tasks.slice(prevTasksCount, showingTasksCount)
    .forEach((task) => renderElement(`.board__tasks`, createCardTemplate(task), `beforeend`));
